...
const characteristicsSlice = createSlice({
    ...
    reducers: {
        ...
        changeCharacteristic(state, action) {
            state.characteristics[action.payload.index] =
...
export const getCharacteristicById = id => state =>
    state.characteristics.characteristics.find(
...
export const getGetAllCharacteristicsFetchStatus = state =>
    state.characteristics.getAllFetchStatus
export const getGetPostPutDeleteCharacteristicsFetchStatus = state =>
...
export const getIsPending = createSelector(
    ...
    (getAllFetchStatus, getPostPutDeleteFetchStatus) =>
        getAllFetchStatus === 'pending' ||
        getPostPutDeleteFetchStatus === 'pending'
)
export const getIsServerError = createSelector(
    getGetAllCharacteristicsFetchStatus,
    getGetPostPutDeleteCharacteristicsFetchStatus,
    (getAllFetchStatus, getPostPutDeleteFetchStatus) =>
        getAllFetchStatus === 'error' || getPostPutDeleteFetchStatus === 'error'
)
...
export const closeServerErrorModal = () => (dispatch, getState) => {
    ...
    if (getGetPostPutDeleteCharacteristicsFetchStatus(getState()) === 'error') {
        dispatch(
            changeGetPostPutDeleteCharacteristicsFetchStatus('idleAfterError')
        )
    }
}
export const deleteCharacteristic =
    (
        characteristicIds,
        unselectCharacteristic = null,
        clearSelectedCharacteristics = null
    ) =>
    (dispatch, getState) => {
        dispatch(changeGetPostPutDeleteCharacteristicsFetchStatus('pending'))
        Promise.all(
            characteristicIds.map(characteristicId => {
                const idGroupWithDeletedCharacteristic = getCharacteristicById(
                    characteristicId
                )(getState()).characteristicGroup

                return deleteCharacteristicFetch(characteristicId).then(
                    response => {
                        ...
                        if (getBehaviorMode(getState()) === 'FAST_MODE') {
                            ...
                            if (
                                ...
                                const characteristicGroup =
                                    getState().characteristicGroups.characteristicGroups.find(
                                        group =>
                                            group.id ===
                                            idGroupWithDeletedCharacteristic
                                    )
                                const updatedCharacteristicGroup = {
                                    ...characteristicGroup,
                                    characteristics:
                                        characteristicGroup.characteristics.filter(
                                            characteristic =>
                                                characteristic.id !==
                                                characteristicId
                                        ),
                                ...
                                const groupIndex =
                                    getState().characteristicGroups.characteristicGroups.findIndex(
                                        group =>
                                            group.id ===
                                            idGroupWithDeletedCharacteristic
                                    )
            ...
            .then(() => {
                dispatch(
                    changeGetPostPutDeleteCharacteristicsFetchStatus('success')
                )
                dispatch(
                    changeGetPostPutDeleteCharacteristicsFetchStatus('idle')
                )
            })
            .catch(e => {
                console.log(e)
                dispatch(
                    changeGetPostPutDeleteCharacteristicsFetchStatus('error')
                )
                if (getBehaviorMode(getState()) === 'FAST_MODE') {
                    dispatch(getAllCharacteristics())
                    if (
                        getGetAllCharacteristicGroupsFetchStatus(getState()) ===
                        'success'
                    ) {
                        dispatch(getAllCharacteristicGroups())
                    }
                    if (clearSelectedCharacteristics) {
...
export const saveNewCharacteristic =
    characteristic => async (dispatch, getState) => {
        const preparedCharacteristicValues =
            deleteNullValuesFromObject(characteristic)
        if (!preparedCharacteristicValues) return
        dispatch(changeGetPostPutDeleteCharacteristicsFetchStatus('pending'))
        try {
            const responseCharacteristic = await postCharacteristicFetch(
                preparedCharacteristicValues
            )
            dispatch(
                changeGetPostPutDeleteCharacteristicsFetchStatus('success')
            )
            ...
            if (getBehaviorMode(getState()) === 'FAST_MODE') {
                if (
                    getGetAllCharacteristicsFetchStatus(getState()) ===
                    'success'
                ) {
                ...
                if (
                    getGetAllCharacteristicGroupsFetchStatus(getState()) ===
                        'success' &&
                    responseCharacteristic.characteristicGroup
                ) {
                    const characteristicGroup =
                        getState().characteristicGroups.characteristicGroups.find(
                            group =>
...
export const saveChangedCharacteristic =
    (characteristic, initialCharacteristicGroupId, callback) =>
    async (dispatch, getState) => {
        ...
        try {
            ...
            if (getBehaviorMode(getState()) === 'FAST_MODE') {
                const changedCharacteristicGroupId =
                    characteristic.characteristicGroup
                        ? characteristic.characteristicGroup.id
                        : null
                dispatch(
                    changeGetPostPutDeleteCharacteristicsFetchStatus('pending')
                )
                const responseCharacteristic = await getCharacteristicFetch(
                    characteristic.id
                )
                dispatch(
                    changeGetPostPutDeleteCharacteristicsFetchStatus('success')
                ...
                if (
                    ...
                    const characteristicIndex =
                        getState().characteristics.characteristics.findIndex(
                            elem => elem.id === characteristic.id
                        )
                    dispatch(
                        changeCharacteristic({
                ...
                if (
                    ...
                    const initialGroup = getCharacteristicGroupById(
                        initialCharacteristicGroupId
                    )(getState())
                    ...
                    dispatch(
                        changeCharacteristicGroup({
                            ...
                            characteristicGroup: {
                                ...initialGroup,
                                characteristics:
                                    initialGroup.characteristics.filter(
                                        characteristic =>
                                            characteristic.id !==
                                            characteristicId
                                    ),
                    ...
                    const changedGroup = getCharacteristicGroupById(
                        changedCharacteristicGroupId
                    )(getState())
                ...
                if (
                    initialCharacteristicGroupId === null &&
                    changedCharacteristicGroupId
                ) {
                    const changedGroupIndex =
                        getState().characteristicGroups.characteristicGroups.findIndex(
                ...
                if (
                    initialCharacteristicGroupId &&
                    changedCharacteristicGroupId === null
                ) {
                    const initialGroupIndex =
                        getState().characteristicGroups.characteristicGroups.findIndex(
                            group => group.id === initialCharacteristicGroupId
...
export const getCharacteristic =
    (characteristicId, callbackSuccess) => dispatch => {
        dispatch(changeGetPostPutDeleteCharacteristicsFetchStatus('pending'))
