...
const sortMyGoodsList = (allGoods, sortingStatus) =>
    dispatch => {
...
export const changeMyGoodsListSortingStatus = (nameOfField, allGoods, sortingStatus) =>
    dispatch => {
        const sortingField = Object.keys(sortingStatus)[0]
        const sortingValue = sortingStatus[sortingField]

        if (sortingField !== nameOfField) {
            dispatch(setSortingStatus({[nameOfField]: 'down'}))
        }

        if ((sortingField === nameOfField) && (sortingValue === 'down')) {
            dispatch(setSortingStatus({[nameOfField]: 'up'}))
        }
