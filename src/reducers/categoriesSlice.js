...
import {
    deepCopyCategories,
    findCategoryById,
} from '../components/pages/addCategory/categoryOperations'
import {
    deleteCategoryFetch,
...
export const getFetchPostPutDeleteStatus = state =>
    state.categories.fetchPostPutDeleteStatus
export const canFetchGetPostPutDelete = createSelector(
    getFetchStatus,
    getFetchPostPutDeleteStatus,
    (fetchAllCategoriesStatus, fetchPostPutDeleteStatus) =>
        fetchAllCategoriesStatus !== 'pending' &&
        fetchPostPutDeleteStatus !== 'pending'
)
export const getIsServerError = createSelector(
    getFetchStatus,
    getFetchPostPutDeleteStatus,
    (fetchAllCategoriesStatus, fetchPostPutDeleteStatus) =>
        fetchAllCategoriesStatus === 'error' ||
        fetchPostPutDeleteStatus === 'error'
)
...
export const moveCategory =
    (draggingCategoryId, newParentCategoryId, openCategory) =>
    (dispatch, getState) => {
        ...
        putCategoryFetch(categoryAttr)
            .then(response => {
                ...
                if (getBehaviorMode(getState()) === 'FAST_MODE') {
                    let categories = deepCopyCategories(
                        getState().categories.categories
                    )
                    const draggingCategory = findCategoryById(
                        categories,
                        draggingCategoryId
                    )
                    ...
                    const parentCategory = findCategoryById(
                        categories,
                        draggingCategory.parentCategory
                    )

                    const newParentCategory = findCategoryById(
                        categories,
                        newParentCategoryId
                    )
                    if (parentCategory) {
                        parentCategory.subCategories =
                            parentCategory.subCategories.filter(
                                subItem => subItem.id !== draggingCategoryId
                            )
                    } else {
...
export const moveCategoryToRoot =
    draggingCategoryId => (dispatch, getState) => {
        const canFetch = canFetchGetPostPutDelete(getState())
        if (!canFetch) return
        ...
        putCategoryFetch(categoryAttr)
            .then(response => {
                ...
                if (getBehaviorMode(getState()) === 'FAST_MODE') {
                    let categories = deepCopyCategories(
                        getState().categoriesList.categories
                    )
                    const draggingCategory = findCategoryById(
                        categories,
                        draggingCategoryId
...
export const deleteCategories =
    (categoryId, callback) => (dispatch, getState) => {
        const canFetch = canFetchGetPostPutDelete(getState())
        if (!canFetch) return
        ...
        deleteCategoryFetch(categoryId)
            .then(response => {
                ...
                if (getBehaviorMode(getState()) === 'FAST_MODE') {
                    ...
                    const deletingCategory = findCategoryById(
                        categories,
                        parseInt(categoryId)
                    )
                    let parentCategory = findCategoryById(
                        categories,
                        deletingCategory.parentCategory
                    )
                    if (parentCategory) {
                        parentCategory.subCategories =
                            parentCategory.subCategories.filter(
                                subCategory =>
                                    subCategory.id !== parseInt(categoryId)
                            )
                    } else {
                        categories = categories.filter(
...
export const changeCategory = changedCategory => (dispatch, getState) => {
    ...
    if (
        ...
        const oldParentCategory = findCategoryById(
            categories,
            category.parentCategory
        )
        const newParentCategory = findCategoryById(
            categories,
            changedCategory.parentCategory.id
        )
        oldParentCategory.subCategories =
            oldParentCategory.subCategories.filter(
                subCategory => subCategory.id !== changedCategory.id
            )
        newParentCategory.subCategories = [
            ...newParentCategory.subCategories,
            category,
