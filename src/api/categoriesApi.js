...
export const getCategoryFetch = id => {
    return fetch(CATEGORY_PATH + `/${id}`, getRequestOptions()).then(
        response => {
            if (!response.ok) {
...
export const getAllCategoriesFetch = () => {
    return fetch(CATEGORY_PATH, getRequestOptions()).then(response =>
        response.json()
    )
}

export const postCategoryFetch = categoryAttr => {
    return fetch(CATEGORY_PATH, postRequestOptions(categoryAttr)).then(
...
export const putCategoryFetch = categoryAttr => {
    return fetch(
        CATEGORY_PATH + `/${categoryAttr.id}`,
        putRequestOptions(categoryAttr)
    )
}
