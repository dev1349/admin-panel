...
export const getGoodFetch = id => {
    return fetch(GET_GOOD_PATH + '?id=' + id, getRequestOptions()).then(
        response => response.json()
    )
}

export const getAllGoodFetch = () => {
    return fetch(GET_ALL_GOOD_PATH, getRequestOptions()).then(response =>
        response.json()
    )
}

export const putGoodFetch = good => {
    return fetch(PUT_GOOD_PATH, putRequestOptions(good)).then(response =>
        response.json()
    )
}
