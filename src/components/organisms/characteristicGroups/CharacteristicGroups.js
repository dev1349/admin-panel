...
const CharacteristicGroups = ({
    ...
    headLines,
    characteristicsStringLength = 40,
    onSelect,
    selectedCharacteristicGroups,
    order,
    orderBy,
    onRequestSort,
    onEditCharacteristicGroup,
}) => {
            ...
            <AdminPageBorder>
                <Box marginTop7>
                    <Table dialogTable>
                        ...
                        <TableBody>
                            {stableSort(characteristicGroups, getComparator(order, orderBy)).map((characteristicGroup, index) => (
                                <TableRow
                                    hover
                                    dialogRow
                                    key={characteristicGroup.id}
                                    dialogSelectedRow={isSelected(characteristicGroup.id, selectedCharacteristicGroups)}
                                ...
                                >
                                    <TableCell align={aligns[0]} dialogCell>
                                        <Checkbox
                                            dialogCheckbox
                                            disableRipple
                                            checked={isSelected(characteristicGroup.id, selectedCharacteristicGroups)}
                                            onChange={onSelect(characteristicGroup.id)}
                                        />
                                    </TableCell>
                                    ...
                                    <TableCell align={aligns[2]} dialogCell>
                                        {characteristicGroup.orderNumber}
                                    </TableCell>
                                        ...
                                        <Fade in={index === activeRow} timeout={0}>
                                            <div>
                                                <IconButton
                                                    dialogButton
                                                    disableRipple
                                                    onClick={onEditCharacteristicGroup(characteristicGroup.id)}
                                                >
                                                    <EditIcon />
                                                </IconButton>
                                            </div>
                    ...
                    {!characteristicGroups.length ? (
                        <Box marginTop7>
                            <Typography mainAdminText textAlignCenter component={'div'}>
                                Список групп характеристик пуст :(
                            </Typography>
                        </Box>
                    ) : (
                        ''
...
export default CharacteristicGroups
