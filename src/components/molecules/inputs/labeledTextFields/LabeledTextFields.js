    ...
    return (
        <LabeledTextFieldsStyled>
            <LabeledInputTemplate>
                <InputLabel htmlFor={id}>{characteristicValues.length ? label : ''}</InputLabel>
                <Box>
                    {characteristicValues.map((value, index) => (
                        <Box key={index} marginTop7={index !== 0} onMouseEnter={handleMouseEnter(index)} onMouseLeave={handleMouseLeave}>
                            <FlexTemplate gap7 alignItemsCenter>
                                <Box flexGrow1>
                                    <TextField
                                        ...
                                        fullWidth
                                        name={index.toString()}
                                        value={characteristicValues[index].value}
                                        onChange={changeCharacteristicValue}
                                        placeholder={'Введите значение характеристики'}
                                        autoFocus={characteristicValues.length === index + 1}
                                    />
                                </Box>
                                <Fade in={activeField === index} timeout={0}>
                                    <div>
                                        <IconButton dialogButton disableRipple onClick={deleteCharacteristicValue(index)}>
                                            <DeleteIcon />
                                        </IconButton>
                                    </div>
                                </Fade>
                    ...
                    <Box marginTop7={characteristicValues.length !== 0}>
                        <Button dialogButton disableRipple onClick={addCharacteristicValue} startIcon={<AddIcon dialogIcon />}>
