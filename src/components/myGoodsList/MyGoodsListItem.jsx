...
const MyGoodsListItem = ({ good }) => {
    return (
        <Tr>
            ...
            <Td centered>{!good.imageStatus && <Img src={defaultImage} />}</Td>
            <Td>{good.name}</Td>
            <Td>{good.model}</Td>
            <Td centered>
                <CellList>
            ...
            <Td>
                <CellList>
                    {printCategory(good.categorySet).map((item, index) => {
                        return <CellListItem key={index}>{item}</CellListItem>
                    })}
                </CellList>
            </Td>
            <Td centered>
                <Label attention={good.count <= 5}>{good.count}</Label>
            </Td>
            <Td>{good.status ? 'Включенко' : 'Не включено'}</Td>
