...
import { getAllClients, getClientsQuantity, getItemsPerPage, setItemsPerPage } from '../../../reducers/clientsSlice'
...
const ClientsPage = () => {
    ...
    const handleChangeItemsPerPage = newValue => dispatch(setItemsPerPage(newValue))
    ...
    const allSelectedClientsIndeterminate = clients && selectedClients.length > 0 && selectedClients.length < clientIds?.length
    const allSelectedClientsChecked = clients && selectedClients.length > 0 && selectedClients.length === clientIds?.length
        ...
        <>
            <>
                ...
                <AdditionalClientFunctions
                    ...
                    allSelectedClientsIndeterminate={allSelectedClientsIndeterminate}
                    allSelectedClientsChecked={allSelectedClientsChecked}
                    isSelectedClients={isSelectedClients}
                    createNewClientLink={'/clients/add'}
                    clients={clients}
                />
                <ClientList clients={clients} selectedClients={selectedClients} setSelectedClients={setSelectedClients} />
                <ItemsPerPage
                ...
                {clientsQuantity > itemsPerPage && (
                    <Pagination count={Math.ceil(clientsQuantity / itemsPerPage)} pathName={'/clients/pages'} />
                )}
            </>
        </>
    )
