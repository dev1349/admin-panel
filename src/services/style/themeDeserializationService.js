export const themeDeserialize = theme => {
    ...
    theme.components &&
        theme.components.forEach(component => {
            const newComp = [component.name, componentDeserialize(component.styleOverrides, component.variants)]
            compArr.push(newComp)
        })
    return {
        components: Object.fromEntries(compArr),
        constants: constantsDeserialize(theme.constants),
    }
}

const constantsDeserialize = constants => {
    return Object.fromEntries(constants.map(x => [x.key, x.value]))
}
...
const styleOverridesDeserialize = styleOverrides => {
    ...
    styleOverrides &&
        styleOverrides.forEach(styleOverride => {
            const newStyleOverride = [styleOverride.name, stylesSODeserialize(styleOverride.styles)]
...
const stylesSODeserialize = stylesSO => {
    const styleSOArr = []
    stylesSO &&
        stylesSO.forEach(styleSO => {
            const newStyleSO = [styleSO.key, valueDeserialize(styleSO.value)]
...
const stylesVDeserialize = stylesV => {
    const styleVArr = []
    stylesV &&
        stylesV.forEach(styleV => {
            const newStyleV = [styleV.key, valueDeserialize(styleV.value)]
