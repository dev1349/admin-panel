...
export const postAttribute = attribute => {
    return (dispatch, getState) => {
        ...
        attribute.attributeGroup =
            getState().attributeGroup.attributeGroupItems.find(el => {
                return el.id === +editAttribute.attributeGroup
            })
        postAttributeFetch(attribute).then(
            // data => {
            // dispatch({
...
export const deleteAttribute = attribute => {
    return dispatch => {
        deleteAttributeFetch(attribute).then(data => {
...
export const getAllAttribute = () => {
    return dispatch => {
        getAllAttributeFetch().then(data => {
            dispatch({
...
export const editAttribute = attributeId => (dispatch, getState) => {
    const currentAttribute = getState().attributes.allAttribute.find(
        item => item.id === attributeId
    )
    Object.keys(currentAttribute).forEach(key =>
        dispatch(changeAddAttributeFields({ [key]: currentAttribute[key] }))
    )
}
...
export const saveEditAttribute = () => (dispatch, getState) => {
    ...
    putAttributeFetch(editAttribute).then(() => {
        dispatch(getAllAttribute())
    })
    editAttribute.name = '' // Очистка поля после сохранения
}
